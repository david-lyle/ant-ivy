...
public class HttpclientURLHandlerTest {
    ...
    {
        defaultTimeoutConstraint = new NamedTimeoutConstraint(
                "default-http-client-handler-timeout");
    ...
    public void testIsReachable() throws Exception {
        ...
        assertFalse("URL resource was expected to be unreachable", handler.isReachable(
            new URL("http://www.google.fr/unknownpage.html"), defaultTimeoutConstraint));
    }
    ...
    public void testGetURLInfo() throws Exception {
        ...
        URLInfo info = handler.getURLInfo(
            new URL(DEFAULT_M2_ROOT
    ...
    public void testContentEncoding() throws Exception {
        assertDownloadOK(new URL("http://carsten.codimi.de/gzip.yaws/daniels.html"),
            new File(testDir, "gzip.txt"));
        assertDownloadOK(
            new URL("http://carsten.codimi.de/gzip.yaws/daniels.html?deflate=on&zlib=on"),
            new File(testDir, "deflate-zlib.txt"));
        ...
        assertDownloadOK(new URL("http://carsten.codimi.de/gzip.yaws/a5.ps?deflate=on"),
            new File(testDir, "a5-deflate.ps"));
        assertDownloadOK(new URL("http://carsten.codimi.de/gzip.yaws/nh80.pdf"),
            new File(testDir, "nh80-gzip.pdf"));
        assertDownloadOK(new URL("http://carsten.codimi.de/gzip.yaws/nh80.pdf?deflate=on"),
            new File(testDir, "nh80-deflate.pdf"));
    }

    /**
     * Tests that the {@link HttpClientHandler}, backed by {@link CredentialsStore Ivy credentials
     * store} works as expected when it interacts with a HTTP server which requires authentication
    ...
    public void testCredentials() throws Exception {
        ...
        final InetSocketAddress serverBindAddr = new InetSocketAddress("localhost",
                TestHelper.getMaybeAvailablePort());
        ...
        try (final AutoCloseable server = TestHelper.createBasicAuthHttpServerBackedRepo(
            serverBindAddr, contextRoot, repoRoot, realm,
    ...
    private void assertDownloadOK(final URL url, final File file) throws Exception {
        ...
        assertTrue(
            "Unexpected content at " + file + " for resource that was downloaded from " + url,
